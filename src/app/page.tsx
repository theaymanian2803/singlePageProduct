...
const colorVariant: ProductVariable[] = [
  {
    name: "Red Socks",
    image: redSocks,
    value: "red",
...
export default function Home() {
  const [selectedColor, setSelectedColor] = useState<ProductVariable>(colorVariant[0]);
  const [hoveredColor, setHoveredColor] = useState<ProductVariable | null>(null);

  const currentImage = hoveredColor ? hoveredColor.image : selectedColor.image;
            ...
            <div className="space-y-6">
              <Card className=" overflow-hidden bg-gradient-card  shawdow-lg">
                <div className="aspect-square bg-white/80 backdrop-blur-sm p-8 flex items-center justify-center">
                  <Image className="w-full h-full object-contain transition-all duration-500 ease-spring transform hover:scale-105" src={currentImage} alt={`${currentImage.name} premium socks`} width={400} height={400}/>
                </div>
              </Card>
                ...
                <div className="flex gap-4">
                  {colorVariant.map((variant) => (
                      <button
                          ...
                          className={`
                      relative w-12 h-12 rounded-full border-2 transition-all duration-300 ease-smooth
